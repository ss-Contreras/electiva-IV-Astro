...
const PacienteComponent: React.FC = () => {
  ...
  const fetchOdontologos = async () => {
    try {
      const response = await axios.get<Odontologo[]>('https://localhost:7027/api/odontologo');
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.post('https://localhost:7027/api/paciente', payload);
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.put(`https://localhost:7027/api/paciente/${pacienteToEdit.id}`, payload);
  ...
  const handleDelete = async (id: number) => {
    ...
    setActionLoading(true);
    try {
      const response = await axios.delete(`https://localhost:7027/api/paciente/${id}`);
      if (response.status === 200) {
        await fetchPacientes();
